...
const Cart = {
    ...
    `
            ...
            $$('.cart__product').forEach((item) => {
                item.querySelector(".icon-delete").onclick = function (e) {
                    ...
                    let itemDeleted = _this.dataCartProduct.filter((item) => item.product.title === titleItem)
                ...
                item.querySelector('.add').onclick = function () {
                    ...
                    _this.status = 1;
                    let titleItem = item.querySelector('.name__product_order').textContent;
                    let itemUpdate = _this.dataCartProduct.filter((item) => item.product.title === titleItem)
    ...
    handleDeleteItem: async function (item) {
        ...
        try {
            const data = await fetchData("/carts/item/66063f8e346942e815a9cd3c", "POST", convertToString({
    ...
    handleUpdateItem: async function (item) {
        ...
        try {
            const data = await fetchData("/carts/update/66063f8e346942e815a9cd3c", "POST", convertToString({
                variation_id: variationId !== null ? variationId : null,
                product_id: productId,
            ...
            if (quantity++ <= inventoryItem || quantity-- > 0) {
                this.dataCartProduct = data.payload.cart_products
                let tempItem = this.dataCartProduct.filter((item) => item.product._id === productId)
                this.quantityItem.textContent = `${tempItem[0].quantity}`;
                this.handleGoSeeOrder()
            } else {
